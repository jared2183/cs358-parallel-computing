						...
						int& rowsPerProc, int& leftOverRows)
{
	...
	if (myRank == 0)  // Master:
	{
		...
		MPI_Bcast(params, sizeof(params) / sizeof(params[0]), MPI_INT, 0 /*master*/, MPI_COMM_WORLD);
		...
		// main version of scatter, scatter also allocates chunk to main process
		MPI_Scatter(image[leftOverRows], rowsPerProc * cols * 3, MPI_UNSIGNED_CHAR,
		           MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, 0 /*master*/, MPI_COMM_WORLD);
	...
	else  // Workers:
	{
		...
		// worker version of scatter, ignores first 3 arguments since they are related to sender/main process
		MPI_Scatter(NULL, 0, MPI_DATATYPE_NULL,
		            image[1], rows * cols * 3, MPI_UNSIGNED_CHAR, 0 /*master*/, MPI_COMM_WORLD);
	}
	                 ...
	                 int rowsPerProc, int leftOverRows)
{
	...
	if (myRank > 0)  // workers:
	{
		...
		// worker version of gather, sends chunk back to main process. last 4 arguments are related to receiving chunks and are ignored
		MPI_Gather(image[1], rows * cols * 3, MPI_UNSIGNED_CHAR,
		           NULL, 0, MPI_DATATYPE_NULL, 0 /*master*/, MPI_COMM_WORLD);
	...
	else  // master:
	{
		...
		// main version of gather, gathers all chunks from workers. first 3 arguments are related to sending chunks and are ignored (need to specify MPI_IN_PLACE)
		MPI_Gather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL,
		           image[leftOverRows], rowsPerProc * cols * 3, MPI_UNSIGNED_CHAR, 0 /*master*/, MPI_COMM_WORLD);
	}
